...
NUMBER_OF_SHADING_MAPS = 100
#%% Gaussian-like shading maps function
...
ADJACENCY = False
filename = 'evaluation2'

#%% Simulate AS configurations
series_data = []
for h in range(0, len(eval_series)):
    circuit = TCT_interconnection(6, 10, eval_series[h])
    circuit.V('input', 1, circuit.gnd, 0)
    simulator = circuit.simulator(temperature=25, nominal_temperature=25)
    analysis = simulator.dc(Vinput=slice(0,50,0.01))

    plt.plot(np.array(analysis.sweep),np.array(analysis.Vinput)*np.array(analysis.sweep), linewidth=0.5)
    plt.xlim(0,50)
    plt.ylim(0,100)
    
    power = np.array(analysis.Vinput)*np.array(analysis.sweep)
    mpp = power.max()
    vmp = analysis.sweep[power.argmax()]
    imp = analysis.Vinput[power.argmax()]
    
    series_data.append((mpp, vmp, imp))
...
for h in range(75, 100):
    print("Starting Shading Map", h)
    shading_map = eval_series[h]
    ...
    for i in range(0, PARTITION_ITERATIONS):
        ...
        for j in range(0, BLOCK_ITERATIONS):
            ...
            for cell_connection in range(0, CELL_ITERATIONS):
                ...
                data_list.append([h, partition, superstring, mpp, vmp, imp, voc, isc])
...
df = pd.DataFrame(data = data_list, columns = [h, "Partition List", "SuperString", "MPP (W)", "VMP (V)",\
                                             "IMP (A)", "VOC (V)", "ISC (A)"])
